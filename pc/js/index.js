...
$(function(){
	...
	$(window).scroll(function(e){
		...
		if(sTop >= navPos){
			if(!(nav.hasClass('fix'))){
				nav.addClass('fix');
				$('.banner').css('margin-bottom',navHeight);//banner下面空出等高占位
			}
		} else {
			if((nav.hasClass('fix'))){
				nav.removeClass('fix');
		...
		//海量正版免费下载
		var introPos={
			start:$('#post_intro').offset().top - navHeight,
			end:$('#post_intro').offset().top+$('#post_intro').outerHeight() - navHeight
		}
		...
		if(sTop >=  introPos.start && sTop < introPos.end){
			...
			if(($('.intro').hasClass('active'))){
				$('.intro').removeClass('active');//高亮
			}
		}
		
		//无需苹果账号
		var usagePos={
			start:$('#post_usage').offset().top - navHeight,
			end:$('#post_usage').offset().top + $('#post_usage').outerHeight() - navHeight
		}
